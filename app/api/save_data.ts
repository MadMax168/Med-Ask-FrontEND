  ...
  try {
    ...
    if (!ehr_data || !chat_history || !Array.isArray(chat_history)) {
      return res
        ...
        .json({
          error:
            "EHR data and chat history are required. Chat history must be an array.",
        });
    }
    ...
    const { error: complaintError } = await supabase
      .from("ChiefComplaint")
      .insert(
        ehr_data.chief_complaint.map((complaint: string) => ({
    ...
    const { error: illnessError } = await supabase
      .from("PresentIllness")
      .insert(
        ehr_data.present_illness.map((desc: string) => ({
          patient_id: patientId,
          description: desc,
        }))
      );
    ...
    const { error: pastIllnessError } = await supabase
      .from("PastIllness")
      .insert(
        ehr_data.past_illness.map((illness: string) => ({
          patient_id: patientId,
          illness,
        }))
      );
    ...
    const { error: familyHistoryError } = await supabase
      .from("FamilyHistory")
      .insert(
        ehr_data.family_history.map(
          (history: { relation: string; condition: string }) => ({
            patient_id: patientId,
    ...
    res
      .status(200)
      .json({ message: "Data and chat history saved successfully!" });
  } catch (error) {
